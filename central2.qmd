...
### Geometric mean for frequency table data
...
where $x_{i}$ is value and if it is a grouped frequency table $x_{i}$ will be the mid value, $f_{i}$ is the frequency , *k* is the number of classes
...
|:----------|:----|:----|:----|:----|:----|:----|
| x         | 20  | 21  | 22  | 23  | 24  | 25  |
| frequency | 4   | 2   | 7   | 1   | 3   | 1   |

: Model data for harmonic mean calculation {#tbl-tomato}
...
**Solution 5.6**:\
To solve the cistern problem, we first calculate the rate at which each pipe fills the cistern. Pipe A fills the cistern in 3 hours, so its rate is $\frac{1}{3}$ of the cistern per hour. Pipe B fills the cistern in 5 hours, so its rate is $\frac{1}{5}$ per hour. To find the combined rate, we add these rates together:

$\frac{1}{3} + \frac{1}{5} = \frac{8}{15}$

This means the two pipes together fill $\frac{8}{15}$ of the cistern each hour. To find the total time, we take the reciprocal of the combined rate: $\text{Total time} = \frac{1}{\frac{8}{15}} = \frac{15}{8} = 1.875$\
To convert 1.875 hours into hours and minutes, we first separate the whole number from the decimal part.

1.875 hours consists of 1 hour (the whole number) and 0.875 hours (the decimal part).

Next, we convert the decimal part into minutes. Since 1 hour is equal to 60 minutes, we multiply 0.875 by 60 *i*.*e*. $0.875 \times 60 = 52.5 \, \text{minutes}$. Rounding 52.5 minutes gives approximately 53 minutes. Thus, 1.875 hours is equivalent to **1 hour and 53 minutes**.\
This can also be easily found using the harmonic mean, the harmonic mean of two pipes is calculated using the formula:

$$HM = \frac{2 \cdot 3 \cdot 5}{3 + 5} = \frac{30}{8} = 3.75 \, \text{hours}$$
...
To find the average speed when traveling the same distance at two different speeds, we use the harmonic mean. The formula for the average speed when the distance is constant but the speeds differ is: $\frac{2 \cdot S_1 \cdot S_2}{S_1 + S_2}$, where $S_1 = 60 \, \text{km/h}$ is the speed from City A to City B, $S_2 = 90 \, \text{km/h}$ is the speed from City B to City A.
...
Consider a semi circle with diameter a+b. Then its radius is half the diameter, which is the arithmetic mean $\frac{a+b}{2}$. See @fig-semiam 
...
Now draw a line from the top of redline which is GM in @fig-semigm to the center of the circle. Now that line is the radius of the circle, so it is equal to AM, which is now the hypotenuse. So from @fig-semiamgm it is clear that  
...
Now if we draw an altitude to the hypotenuse @fig-semiamgm, the upper length on the hypotenuse is the harmonic mean as shown in @fig-semiamgmhm. As you can see that HM is a leg of a triangle where the GM is the hypotenuse, the GM is never smaller than the HM. So $$\mathbf{GM \geq HM}$$ {#eq-gmhm}    
...
$$\mathbf{Q}_{\mathbf{1}}\mathbf{=}\mathbf{20 +}\frac{\mathbf{51 - 29}}{\mathbf{25}}\mathbf{\times 10\  = 28.8}$$
...
$$\mathbf{Q}_{\mathbf{1}}\mathbf{=}\mathbf{60 +}\frac{\mathbf{153 - 145}}{\mathbf{22}}\mathbf{\times 10 = 63.63}$$
...
2.  Find the position of the percentile.
    For the $p$-th percentile, calculate the position using the formula: $$
...
2.  Find $\left( \frac{\text{p.n}}{100} \right)$

3.  See in the cumulative frequencies, the value just greater than$\left( \frac{\text{p.n}}{100} \right)$and then the corresponding class interval is called **Percentile class**.

4.  Use the following formula

$$\mathbf{P}_{\mathbf{p}}\mathbf{= l +}\frac{\left( \frac{\mathbf{p \times n}}{\mathbf{100}} \right)\mathbf{- cf}}{\mathbf{f}}\mathbf{\times c}$$ {#eq-percentile1}
...
**Example 5.11**: Compute $\mathbf{P}_{\mathbf{25}}$ and $\mathbf{P}_{\mathbf{75}}$ for the data 25, 18, 30, 8, 15, 5, 10, 35, 40, 45
...
$\mathbf{D}_{\mathbf{d}}\mathbf{=}\left( \frac{\mathbf{d}\left( \mathbf{n + 1} \right)}{\mathbf{10}} \right)^{\mathbf{\text{th}}}$  item. where $d$ is the decile number (e.g., $d = 1$ for the first decile, $d = 9$ for the ninth decile), and $n$ is the total number of data points. 
...
For example, to find the first decile ($D_1$):
- Calculate $D_1 = \left( \frac{1(n + 1)}{10} \right)$.
- Locate the cumulative frequency that is just greater than $D_1$, and the corresponding value of $x$ is $D_1$.  
...
2.  Find $\left( \frac{\text{d.n}}{10} \right)$

3.  See in the cumulative frequencies, the value just greater than$\left( \frac{\text{d.n}}{10} \right)$and then the corresponding class interval is called **decile class**.

4.  Use the following formula

$$\mathbf{D}_{\mathbf{d}}\mathbf{= l +}\frac{\left( \frac{\mathbf{p \times n}}{\mathbf{10}} \right)\mathbf{- cf}}{\mathbf{f}}\mathbf{\times c}$$ {#eq-decile}
